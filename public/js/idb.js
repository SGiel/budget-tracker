...
// ?? onupgradeneeded a listener??
request.onupgradeneeded = function(event) {
  // save a reference to the database 
  const db = event.target.result;
  // create an object store (table) called `new_budget`, set it to have an 
  // auto incrementing primary key of sorts 
  // ?? is creatObjectStore a method from indexedDB??

  // this db is a local variable
...
function uploadBudget() {
  ...
  getAll.onsuccess = function () {
    ...
    if (getAll.result.length > 0) {
      // ?  IS THIS supposed to be the api route for posting many transactions??
